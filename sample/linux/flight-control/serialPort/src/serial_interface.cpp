...
void SerialInterface::YunTaiSmallDeltaSet(float pitch,float yaw){
    SerialPacket sendPacket;
    sendPacket.creatPacket(CMD_SERIAL_YUNTAI_SMALL_DELTA_SET);
    sendPacket.setFloatInBuffer(pitch,2);
    sendPacket.setFloatInBuffer(yaw,6);
    sendPacket.setUncharInBuffer(0x00,10);
    dataSend(sendPacket);
...
void SerialInterface::YunTaiSmallAbsSet(float pitch ,float yaw ){
    SerialPacket sendPacket;
    sendPacket.creatPacket(CMD_SERIAL_YUNTAI_SMALL_ABS_SET);
    sendPacket.setFloatInBuffer(pitch,2);
    sendPacket.setFloatInBuffer(yaw,6);
    sendPacket.setUncharInBuffer(0x00,10);
    dataSend(sendPacket);   
...
void SerialInterface::getAbsYunTaiSmallDelta(){
    SerialPacket recvPacket;
    recvPacket.creatPacket(CMD_SERIAL_ABS_YUNTAI_SMALL_DELTA);
    dataSend(recvPacket);
}

void SerialInterface::getRelaYunTaiDelta(){
    SerialPacket recvPacket;
    recvPacket.creatPacket(CMD_SERIAL_RELA_YUNTAI);
    dataSend(recvPacket);
}
...
void SerialInterface::YunTaiSmallShoot(unsigned char num){
    SerialPacket sendPacket;
    sendPacket.creatPacket(CMD_SERIAL_SMALL_SHOOT);
    sendPacket.setUncharInBuffer(num,2);
    dataSend(sendPacket);
}
